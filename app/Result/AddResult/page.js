...
const page = () => {
    ...
    const handleFileChange = (e) => {
        ...
        setSelectedFiles((prevData) =>[...prevData , ...filesData]);
    }
    const handleClear = () => {
        setSelectedFiles([]);
    }
    ...
    const handleUpload = async() =>{
        ...
        const uploadPromises = selectedFiles.map(async (fileData) => {
            ...
            const key = push(resultRef).key;
            const resultDbRef =ref(db, `/pdf/${key}`);
            const resultStorageRef = Sref(storage, '/pdf'+ `/${fileData.name}`)

            await uploadBytes(resultStorageRef, file);
            const downloadURL = await getDownloadURL(resultStorageRef);
            const Data ={
                pdfTitle: fileData.name,
                pdfUrl: downloadURL,
                resultName: fileData.name,
                uniqueKey: key,
            }
            await set(resultDbRef, Data);
          });
          try {
